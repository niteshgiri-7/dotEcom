                    ...
                    <ul>
                        {listItems.map((item, idx) => (
                            <Link to={item.url} key={idx}>
                                <li className={`flex gap-2 px-2 py-2 items-center mt-2 md:mt-4 ${item.url===location.pathname?"bg-blue-400 bg-opacity-20  border rounded-md":""} text-[rgba(0,0,0,0.8)]`}>
                                    {<item.icon />}
                                    <span>{item.text}</span>
                                </li>
                            </Link>
